...
def show_answer_provenance_graph(
    ...
    - 🟡 Answer
    - 📦 Chunks
    - Edges: Query → Chunk (sim_q), Chunk → Answer (sim_a)
    - (optional) future: entities + KG edges
    """
    import streamlit as st
    from pathlib import Path
    from pyvis.network import Network

    if not polished_answer.strip() or not rows_with_answer_sim:
    ...
    #   - extract_entities from chunk texts
    #   - connect entities to chunks and to Answer
...
def show_chunk_entity_output_graph(
    ...
    Graph:
      ...
      - Edges:
          ...
          Entity ↔ Entity (KG edges from Neo4j, 1–2 hops)
    """
    import streamlit as st
    ...
    for (idx, doc, sim_q, sim_a) in rows_with_answer_sim:
        ...
        # --- edge: Query → Chunk (similarity to query) ---
        try:
            sim_q_f = float(sim_q)
        except Exception:
            sim_q_f = 0.0
...
def show_chunk_entity_output_graph2(
    ...
    Graph:
      ...
      - Edges:
          Chunk → OP         (similarity to answer)
          Chunk → Entity     (MENTIONS)
          OP → Entity        (importance wrt answer; best a_sim)
          Entity ↔ Entity    (KG edges)
    """
    import streamlit as st
    ...
    chunk_entities = defaultdict(set)
    all_entities = set()
    ent_best_ans_sim = defaultdict(float)

    for (idx, doc, _, sim_a) in rows_with_answer_sim:
        text = getattr(doc, "page_content", "") or ""
        meta = dict(getattr(doc, "metadata", {}) or {})
...
if not q.strip():
    ...
    # ---------- NEW: Answer Provenance Graph ----------
    st.subheader("🧬 Answer Provenance Graph")

    # Embed the answer once
    answer_vec = embedder.embed_query(final_answer)

    rows_with_answer_sim = []
    for i, (doc, sim_q) in enumerate(rows, 1):
        chunk_text = getattr(doc, "page_content", "") or ""
        chunk_vec = embedder.embed_query(chunk_text)
        sim_a = _cosine(answer_vec, chunk_vec)
        rows_with_answer_sim.append((i, doc, sim_q, sim_a))

    # Use KG only if enabled + connected (same pattern you already use)
    provenance_neo = neo_driver if (KG_ENABLED and neo_driver) else None

    show_answer_provenance_graph(
        q,
        final_answer,
        rows_with_answer_sim,
    ...
    show_chunk_entity_output_graph(
    q,
    final_answer,
    rows_with_answer_sim,
    neo_driver=provenance_neo,
